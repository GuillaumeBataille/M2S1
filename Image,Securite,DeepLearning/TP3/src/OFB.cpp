...
int main(int argc, char *argv[])
{
    ...
    out = aes.EncryptOFB(Img, plainLen, key, iv);
    bool noise = true;
    if (noise)
        for (int i = 0; i < nTaille; i++)
        {
    ...
    // Ecriture
    ecrire_image_pgm("../out/OFB_C.pgm", ImgOut, nH, nW);
    std::cout << "Pour chiffrage AES méthode OFB :" << std::endl;
    ...
    ImageAlgorithms::writeHistoDatFile(ImgIn, nTaille, "../dat/OFB/original.dat", false);
    ImageAlgorithms::writeHistoDatFile(ImgOut, nTaille, "../dat/OFB/crypt.dat", false);

    innew = aes.DecryptOFB(out, plainLen, key, iv);
    for (int i = 0; i < nTaille; i++)
    {
        ImgOut[i] = innew[i];
        ImgNoise[i] = OCTET(ImgIn[i] - ImgOut[i]);
    }
    ecrire_image_pgm("../out/OFB_D.pgm", ImgOut, nH, nW);
    ecrire_image_pgm("../out/OFB_N.pgm", ImgNoise, nH, nW);

    // Infos
    std::cout << "Pour déchiffrage AES méthode OFB :" << std::endl;
    std::cout << "Entropy de base: " << ImageAlgorithms::entropy(ImgIn, nTaille) << " bits/pixel" << std::endl;
    ...
    ImageAlgorithms::writeHistoDatFile(ImgOut, nTaille, "../dat/OFB/decrypt.dat", false);
