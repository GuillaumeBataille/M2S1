...
int main(int argc, char *argv[])
{
    ...
    unsigned int plainLen = nTaille * sizeof(unsigned char);                                                                // bytes in plaintext
    unsigned char *out, *innew;
    AES aes(AESKeyLength::AES_128); ////128 - key length, can be 128, 192 or 256
    out = aes.EncryptECB(Img, plainLen, key);
    srand(out[0]);
    ...
    // Ecriture
    ecrire_image_pgm("../out/ECB_C.pgm", ImgOut, nH, nW);
    std::cout << "Pour chiffrage AES méthode ECB :" << std::endl;
    ...
    ImageAlgorithms::writeHistoDatFile(ImgIn, nTaille, "../dat/ECB/original.dat", false);
    ImageAlgorithms::writeHistoDatFile(ImgOut, nTaille, "../dat/ECB/crypt.dat", false);

    innew = aes.DecryptECB(out, plainLen, key);
    for (int i = 0; i < nTaille; i++)
    {
        ImgOut[i] = innew[i];
        ImgNoise[i] = OCTET(ImgIn[i] - ImgOut[i]);
    }
    ecrire_image_pgm("../out/ECB_D.pgm", ImgOut, nH, nW);
    ecrire_image_pgm("../out/ECB_N.pgm", ImgNoise, nH, nW);

    // Infos
    std::cout << "Pour déchiffrage AES méthode ECB :" << std::endl;
    std::cout << "Entropy de base: " << ImageAlgorithms::entropy(ImgIn, nTaille) << " bits/pixel" << std::endl;
    ...
    ImageAlgorithms::writeHistoDatFile(ImgOut, nTaille, "../dat/ECB/decrypt.dat", false);
