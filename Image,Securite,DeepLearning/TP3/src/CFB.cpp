...
int main(int argc, char *argv[])
{
    ...
    out = aes.EncryptCFB(Img, plainLen, key, iv);
    bool noise = true;
    srand(out[0]);
    if (noise)
        for (int i = 0; i < nTaille; i++)
    ...
    // Ecriture
    ecrire_image_pgm("../out/CFB_C.pgm", ImgOut, nH, nW);
    std::cout << "Pour chiffrage AES méthode CFB :" << std::endl;
    ...
    ImageAlgorithms::writeHistoDatFile(ImgIn, nTaille, "../dat/CFB/original.dat", false);
    ImageAlgorithms::writeHistoDatFile(ImgOut, nTaille, "../dat/CFB/crypt.dat", false);

    innew = aes.DecryptCFB(out, plainLen, key, iv);
    for (int i = 0; i < nTaille; i++)
    {
        ImgOut[i] = innew[i];
        ImgNoise[i] = OCTET(ImgIn[i] - ImgOut[i]);
    }
    ecrire_image_pgm("../out/CFB_D.pgm", ImgOut, nH, nW);
    ecrire_image_pgm("../out/CFB_N.pgm", ImgNoise, nH, nW);

    // Infos
    std::cout << "Pour déchiffrage AES méthode CFB :" << std::endl;
    std::cout << "Entropy de base: " << ImageAlgorithms::entropy(ImgIn, nTaille) << " bits/pixel" << std::endl;
    ...
    ImageAlgorithms::writeHistoDatFile(ImgOut, nTaille, "../dat/CFB/decrypt.dat", false);
