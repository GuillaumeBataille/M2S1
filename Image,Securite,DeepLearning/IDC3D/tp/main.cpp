...
std::vector<std::vector<unsigned int>> GenerateBins(Eigen::MatrixXd m, unsigned int NbrOfBits)
{
  ...
  { // Pour chaque vertex, on va voir si ils tombent ou pas dans la range courante
...
void insertion(Eigen::MatrixXd &m, std::vector<bool> message, unsigned int n, double k, double alpha)
{
  ...
  for (int i = 0; i < n; ++i)
  { // Pour chaque bit/bin du message
    ...
    if (message[i])
    { // Si le message i est un 1

      while (mean < (0.5 + alpha))
      {                             // Tant que mean du bin courant < 0.5+alpha
...
int main(int argc, char *argv[])
{
  ...
  igl::readOBJ("./camion.obj", Vertices, Faces);
