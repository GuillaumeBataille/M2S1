...
unsigned char *AES::EncryptOFB(const unsigned char in[], unsigned int inLen,
  ...
  unsigned char *roundKeys = new unsigned char[4 * Nb * (Nr + 1)];
  KeyExpansion(key, roundKeys);
  memcpy(block, iv, blockBytesLen); // block = iv
  
  for (unsigned int i = 0; i < inLen; i += blockBytesLen) {
    EncryptBlock(block, encryptedBlock, roundKeys); // param(in, out, roundKey)
...
unsigned char *AES::DecryptOFB(const unsigned char in[], unsigned int inLen,
                               const unsigned char key[],
                               const unsigned char *iv) {
  return EncryptOFB(in, inLen, key, iv);
}
...
unsigned char *AES::DecryptCFB(const unsigned char in[], unsigned int inLen,
  ...
  memcpy(block, iv, blockBytesLen); // block = iv
  for (unsigned int i = 0; i < inLen; i += blockBytesLen) {
    EncryptBlock(block, encryptedBlock, roundKeys); // encryptedBlock => block & roundkey
    XorBlocks(in + i, encryptedBlock, out + i, blockBytesLen);
    memcpy(block, in + i, blockBytesLen); // block = in+i
  }
