...
class Vec3 {
    ...
    public:
        ...
        void normalize() 
        {
            ...
            if(L == 0){
                std::cout << "error normalize length = 0\n";
                return;
            }
        ...
        Vec3 operator-(const Vec3& other) const {
            return Vec3(mVals[0] - other.mVals[0], mVals[1] - other.mVals[1], mVals[2] - other.mVals[2]);
        }
        ...
        static double dot(const Vec3& a, const Vec3& b) {
            return a.mVals[0] * b.mVals[0] + a.mVals[1] * b.mVals[1] + a.mVals[2] * b.mVals[2];
        }

        Vec3 operator*(double scalar) const {
            return Vec3(mVals[0] * scalar, mVals[1] * scalar, mVals[2] * scalar);
        }
        ...
        static Vec3 projectOnSegment(const Vec3& a, const Vec3& b, const Vec3& point) {
            Vec3 ab = b.operator-(a);
            Vec3 ap = point.operator-(a);
            ...
            if (t < 0.0) {
                ...
                // Point is closest to the end of the segment
                return b;
            } else {
                // Point is closest to a point between a and b
                return a.operator+(ab * t);
            }
        }

        std::string toString(){
            std::string s = std::to_string(mVals[0]) + ", " + std::to_string(mVals[1]) + ", " + std::to_string(mVals[2]);
            return s;
        }
};
...
public:
    ...
    // La fonction utilise atan2 pour couvrir tout les angles possibles et pas seulement 180°
    inline static
    Mat3 getRotationMatrixAligning(const Vec3& vecSrc, const Vec3& vecTarget) {
		Vec3 vecSrcUnit = vecSrc;
		vecSrcUnit.normalize();
		Vec3 vecTargetUnit = vecTarget;
		vecTargetUnit.normalize();
		Vec3 axis = Vec3::cross(vecSrcUnit, vecTargetUnit);

		double angle = atan2(axis.length(), Vec3::dot(vecSrcUnit, vecTargetUnit));
