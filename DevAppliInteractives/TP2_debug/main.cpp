...
void draw () {
    ...
    if( displayMode == 1 ) {
        ...
        // code pour afficher le vertex 1
        // glPointSize(12.0);
        // glBegin(GL_POINTS);
        // glColor3f(1,1,1);
        // glVertex3f (mesh.V[0].p[0], mesh.V[0].p[1], mesh.V[0].p[2]);
        // glEnd();
    }
    ...
    if( displayMode == 2 ) {
        ...
        // code pour afficher la targetPos pour l'IK
        glPointSize(12.0);
        glBegin(GL_POINTS);
        glColor3f(0.3,0.5,0.8);
        glVertex3f (targetPos[0], targetPos[1], targetPos[2]);
        glEnd();
...
void moveTargetArticulationBy( Vec3 translation ) {
    if( targetArticulation > -1  &&  targetArticulation < skeleton.articulations.size() ) {
        Skeleton *s = &skeleton;
        SkeletonTransformation *st = &skeletonTransfoIK;

        //targetPos = skeletonTransfoIK.articulations_transformed_position[ targetArticulation ] + translation;
        targetPos += translation*0.1;

        if(enableIk){
            skeleton.updateIKChain( skeletonTransfoIK, targetArticulation , targetPos );
        }
    }
}
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 's':
        if( displayMode == 2 ) {
            moveTargetArticulationBy( 0.1 * Vec3(0,-1,0) );
        }
        break;
    case 'z':
        if( displayMode == 2 ) {
    ...
    case 'm':
        ...
        std::cout << "displayMode : " <<displayMode << "\n";
        break;
...
int main (int argc, char ** argv) {
    ...
    mesh.loadOFF("models/test.off");
    skeleton.load("models/test.skel");
