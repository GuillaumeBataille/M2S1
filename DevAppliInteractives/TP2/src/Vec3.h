...
public:
    ...
    static Mat3 inverse(Mat3 const &m, bool isRealInverse = true, double defaultValueForInverseSingularValue = 0.0)
    {
    ...
    static Mat3 pseudoInverse(Mat3 const &m, bool &isRealInverse, double defaultValueForInverseSingularValue = 0.0)
    {
        float det = m.determinant();
        if (fabs(det) != 0.0)
        {
    ...
    template <class point_t>
    inline static Mat3 getRotationMatrixFromAxisAndAngle(const point_t &axis, float angle)
    {
    ...
    inline static
        ...
        Mat3
        getRotationMatrixAligning(const Vec3 &vecSrc, const Vec3 &vecTarget)
    {
    ...
    template <class point_t>
    inline static Mat3 tensor(const point_t &p1, const point_t &p2)
    {
        return Mat3(
    ...
    template <class point_t>
    inline static Mat3 vectorial(const point_t &p)
    {
        return Mat3(
            0, -p[2], p[1],
            p[2], 0, -p[0],
